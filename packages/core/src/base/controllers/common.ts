...
export const bindDirectionKeys: (keyboard: KeyboardController, keymap: DirectionKeymap) => Observable<DirectionOutput> =
  (keyboard: KeyboardController, keymap: DirectionKeymap) => {
    ...
    return combineLatest(keys.map(x => keyboard.bindMany(...x)))
      .pipe(
        map((moveDirection) => {
