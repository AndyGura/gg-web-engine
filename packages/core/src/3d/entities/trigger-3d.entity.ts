...
import { Gg3dWorldTypeDocRepo, PhysicsTypeDocRepo3D } from '../gg-3d-world';

export class Trigger3dEntity<PTypeDoc extends PhysicsTypeDocRepo3D = PhysicsTypeDocRepo3D>
  extends IEntity<Point3, Point4, Gg3dWorldTypeDocRepo & { pTypeDoc: PTypeDoc }>
  implements IPositionable3d
{
  ...
  > {
    return this.objectBody.onEntityEntered.pipe(
      map(c => c.entity as IEntity<Point3, Point4, Gg3dWorldTypeDocRepo & { pTypeDoc: PTypeDoc }> & IPositionable3d),
    );
  }
